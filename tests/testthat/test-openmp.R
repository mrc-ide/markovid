test_that("output is identical with vs without multi-threading", {
  ...
  # run MCMC without multi-threading
  set.seed(1)
  message("running without multi-threading")
  t0 <- Sys.time()
  mcmc1 <- markovid::run_mcmc(data_list = data_list,
                             df_params = df_params,
                             burnin = burnin,
                             samples = samples,
                             chains = chains,
  ...
  # run MCMC with multi-threading
  set.seed(1)
  message("running with multi-threading")
  t0 <- Sys.time()
  mcmc2 <- markovid::run_mcmc(data_list = data_list,
                              df_params = df_params,
                              burnin = burnin,
                              samples = samples,
                              chains = chains,
  ...
  # check that output identical between runs
  testthat::expect_true(all.equal(mcmc1, mcmc2))
