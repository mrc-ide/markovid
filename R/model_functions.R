...
sim_indlevel <- function(params_scalar,
  ...
  assert_in(c("s_AI", "s_AD", "s_AC", "s_ID", "s_IS", "s_SC"),
            names(params_scalar))
  assert_list(params_age)
  assert_in(c("p_AI", "p_AD", "p_ID", "m_AI", "m_AC", "m_AD", "m_ID", "m_IS", "m_SC"),
            names(params_age))
  ...
  t_IS = round(rgamma(n, shape = 1/s_IS^2, scale = m_IS[w_age]*s_IS^2))
  ...
  df_sim$date_stepdown[w_IC] <- df_sim$date_admission[w_IC] + t_AI[w_IC] + t_IS[w_IC]
  ...
  df_sim$date_final_outcome[w_IC] <- df_sim$date_stepdown[w_IC] + t_SC[w_IC]
...
#'   generated by \code{sim_indlevel()}), aggregate data by each 1-year age group.
#'
...
#'   (denominator) at each stage. These two outputs are given for each
#'   transition probability. For lengths of stay in each state we require a
#'   different tabulation in terms of the number of patients that were in a
...
aggregate_indlevel <- function(df_data,
  ...
  p_AI_numer <- p_AI_denom <- p_AD_numer <- p_AD_denom <- p_ID_numer <- p_ID_denom <- rep(NA, n_age)
  m_AI_count <- m_AD_count <- m_AC_count <- m_ID_count <- m_IS_count <- m_SC_count <- replicate(n_age, rep(NA, t_max), simplify = FALSE)
